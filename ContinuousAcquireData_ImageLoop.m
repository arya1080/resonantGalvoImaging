% Code for displaying resonant galvo-scanned images from signals fed into the DAQ
% Written by Arya C. Mugdha and Jesse W. Wilson
...
% DAQ.addAnalogInputChannel('PXI1Slot6', 'ai5', 'Voltage'); % photodiode ai5 = RCM 
DAQ.addAnalogInputChannel('PXI1Slot6', 'ai7', 'Voltage'); % photodiode ai7 = pump-probe
...
ny = 128;
count = 0;
%% code-block for calulating the scan phase (x_shift) shift needed to align horizontal line scans correctly 
% fitting routine to calculate the correct x_shift
x_fov = 0.6; % initial guess for x scan range of FOV

% calibration table for pump-probe channel with 10^4 gain
% x_shift_table = [1.5108e-05, 1.5108e-05, 1.4808e-05, 1.4508e-05, 1.3908e-05, 1.3308e-05, 1.3008e-05, 1.3008e-05, 1.2708e-05, 1.2408e-05, 1.2108e-05, 1.1808e-05];

% calibration table for pump-probe channel with 10^5 gain
x_shift_table = [1.5108e-05, 1.4508e-05, 1.4208e-05, 1.3908e-05, 1.3608e-05, 1.3308e-05, 1.3308e-05, 1.3008e-05, 1.2708e-05, 1.2408e-05, 1.2408e-05, 1.2108e-05];

ft = fittype('A + B*x','independent','x');

% for transmission channels
x_range = linspace(0.3,2.5,12);
fitobj = fit(x_range.', x_shift_table.', ft);
x_shift = fitobj(x_fov);

%% set-up figure and callback controls
figure();
clf;
hImageObj = imagesc(0, 0, 0);
colorbar();
colormap(gray);
% caxis([-.2 -0.1]); % color axis limit 
...
%% define data acquisition routine with image interpolation
...
%% define the callback functions used during imaging
